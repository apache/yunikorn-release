...
func (tp *TemplateProcessor) ProcessConfigFile(configPath string) (string, error) {
	...
	err = tmpl.Execute(&buf, tp.params)
	if err != nil {
...
func (tp *TemplateProcessor) defaultParam(paramName interface{}, defaultValue interface{}) interface{} {
	paramStr := tp.toString(paramName)
	...
	if val, exists := tp.params[paramStr]; exists {
		...
		convertedVal := tp.convertToType(tp.toString(val), defaultValue)
		...
		if _, isStringDefault := defaultValue.(string); isStringDefault {
			return tp.toString(convertedVal)
		}
...
func (tp *TemplateProcessor) mathOp(a, b interface{}, op string) interface{} {
	aVal := tp.toFloat(a)
	bVal := tp.toFloat(b)
...
func (tp *TemplateProcessor) toString(val interface{}) string {
	if val == nil {
		return ""
	}
